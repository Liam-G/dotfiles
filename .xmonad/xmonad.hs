...
--
myWorkspaces = ["term","web","media","code", "other"] ++ map show [6..9]
...
myManageHook = composeAll
    [
      fmap( "Limits" `isInfixOf`) title --> doCenterFloat
    , title     =? "Picture-in-Picture"           --> doCenterFloat
    , title =? "Blockify"                     --> doCenterFloat
    ...
    , fmap("Figure" `isInfixOf`) title --> doCenterFloat
	] 
	where 
